...
#define INFO_SWITCH 34    // INFO button
...
#define EXHAUST_PIN 16    // Kipas exhaust
#define PUSHER_PIN 15     // Steam pusher
#define HEATER1_PIN 17    // Heater Hangat
#define HEATER2_PIN 5     // Heater Panas
...
//=============================================================================== PWM & Servo Configuration (FROM KODE PERTAMA - COMPAT v3.3.3) ===============================================================================
...
String generateSessionId()
{
    ...
    struct tm *ptm = localtime(&now);
    char buffer[16];
    sprintf(buffer, "%04d%02d%02d_%02d%02d%02d", ptm->tm_year + 1900, ptm->tm_mon + 1, ptm->tm_mday, ptm->tm_hour, ptm->tm_min, ptm->tm_sec);
    return String(buffer);
}
...
void setup()
{
    ...
    // === PWM KIPAS - LOGIKA DARI KODE PERTAMA (ledcAttach for v3.3.3) ===
    ledcAttach(FANHEATPAN1, PWM_FREQ, PWM_RESOLUTION);
    ledcAttach(FANHEATCOL2, PWM_FREQ, PWM_RESOLUTION);
    ledcAttach(EXHAUST_PIN, PWM_FREQ, PWM_RESOLUTION);
    ledcAttach(PUSHER_PIN, PWM_FREQ, PWM_RESOLUTION);
    Serial.println("PWM channels configured (auto-assign):");
    Serial.println(" Fan Panel: Pin 26 @ 25kHz, 8-bit");
    Serial.println(" Fan Collector: Pin 25 @ 25kHz, 8-bit");
    Serial.println(" Exhaust Fan: Pin 16 @ 25kHz, 8-bit");
    Serial.println(" Pusher Fan: Pin 16 @ 25kHz, 8-bit");
    ...
    // Initialize servo with explicit PWM configuration (FROM KODE PERTAMA)
    ESP32PWM::allocateTimer(SERVO_TIMER);
    exhaustServo.setPeriodHertz(50);
    exhaustServo.attach(SERVO_PIN, 500, 2400);
...
void handleTareButton()
{
    ...
    if (currentReading && !tareButton.lastPressed && (currentTime - tareButton.lastPressTime) > 50)
    { // Debounce
...
void handleStartButton()
{
    ...
    unsigned long currentTime = millis();
    if (currentReading && !startButton.lastPressed && (currentTime - startButton.lastPressTime) > 50)
    {
...
void handleInfoButton()
{
    ...
    unsigned long currentTime = millis();
    if (currentReading && !infoButton.lastPressed && (currentTime - infoButton.lastPressTime) > 50)
    {
...
void handleRestartButton()
{
    ...
    unsigned long currentTime = millis();
    if (currentReading && !restartButton.lastPressed && (currentTime - restartButton.lastPressTime) > 50)
    {
...
void restartSystem()
{
    ...
    Serial.println("Timer system reset - Awaiting web commands");
...
void switchToAutoMode()
{
    ...
    if (systemRunning)
    {
        Serial.println("Resetting actuators for AUTO mode...");

        digitalWrite(HEATER1_PIN, HEATER_OFF);
        digitalWrite(HEATER2_PIN, HEATER_OFF);
        ledcWrite(EXHAUST_PIN, 0);
        exhaustServo.write(0);
        ...
        Serial.println("All actuators reset to safe state");
        ...
        if (processStatus == "Berjalan" && !tempProtectionActive && !doorOpen)
        {
            ...
            float outputExhaust = fuzzy->defuzzify(3);

            controlHeater(outputPemanas);
            controlFan(outputKipas);
            controlExhaust(outputExhaust);
            controlSteamPusher(true);

            Serial.println("Fuzzy logic applied - System now in AUTO control");
        }
    }
...
void switchToManualMode()
{
    ...
    Serial.println("Resetting actuators for MANUAL mode...");
...
void checkTimerFromFirebase()
{
    ...
    if (Firebase.getBool(firebaseData, "/timer/enabled"))
    {
        ...
        if (newEnabled && !timerEnabled)
        {
            ...
            {
                int newRemaining = firebaseData.intData();
                ...
                if (newRemaining > 0)
                {
                    ...
                    Serial.println("  ESP32 will monitor remaining from web");
                    Serial.println("========================================");
        ...
        // CASE 2: TIMER BERJALAN
        else if (newEnabled && timerEnabled)
        {
            delay(50);
            if (Firebase.getInt(firebaseData, "/timer/remaining"))
            {
                int webRemaining = firebaseData.intData();

                if ((int)webRemaining - (int)timerRemaining > 2 || (int)timerRemaining - (int)webRemaining > 2)
                {
                    Serial.println("  Timer update from WEB: " + String(webRemaining) + "s (was " + String(timerRemaining) + "s)");
                    timerRemaining = webRemaining;
                }

                if (webRemaining == 0 && timerRemaining != 0)
                {
                    Serial.println("\n========================================");
                    Serial.println("TIMER COMPLETED NATURALLY!");
                    Serial.println("========================================");
        ...
        // CASE 3: TIMER DIMATIKAN
        else if (!newEnabled && timerEnabled)
        {
            int remainingBeforeStop = 0;
            delay(50);

            if (Firebase.getInt(firebaseData, "/timer/remaining_before_stop"))
            {
                remainingBeforeStop = firebaseData.intData();
            }
            delay(50);

            bool stoppedManually = false;
            if (Firebase.getBool(firebaseData, "/timer/stopped_manually"))
            {
                stoppedManually = firebaseData.boolData();
            }
            ...
            if (stoppedManually)
            {
                ...
                Serial.println("  Remaining before stop: " + String(remainingBeforeStop) + "s");
                Serial.println("========================================");
            ...
            {
                Serial.println("Timer paused by WEB");
                timerEnabled = false;
                timerPaused = true;
                timerRemaining = remainingBeforeStop;
            }
        }
    }
}

// Menjalankan fungsi timer ke akuator
void applyTimerControls()
{
    Serial.println("\n=== TIMER CONTROL MODE (Device Running) ===");
...
void performTare()
{
    ...
    if (!scale.is_ready())
    {
        ...
        Serial.println("X Scale not ready for tare!");
        return;
    }
...
void handleInfoDisplayTimeout()
{
    if (infoDisplayActive && (millis() - infoDisplayStart > INFO_DISPLAY_TIMEOUT))
    {
        infoDisplayActive = false;
        currentInfoMode = INFO_DEFAULT;
        Serial.println("Info display timeout - returning to default");
    }
}
...
void controlExhaustHumidity(float humidity)
{
    ...
    if (excess <= 5.0)
    {
        ledcWrite(EXHAUST_PIN, 170);
        exhaustServo.write(45);
        exhaustStatus = "Sedang-Humid (67%)";
        Serial.println("Exhaust SEDANG - Excess humidity: " + String(excess, 1) + "%");
    }
    else if (excess <= 10.0)
    {
        ledcWrite(EXHAUST_PIN, 215);
        exhaustServo.write(70);
        exhaustStatus = "Cepat-Humid (85%)";
    ...
    else
    {
        ledcWrite(EXHAUST_PIN, 255);
        exhaustServo.write(90);
        exhaustStatus = "Maksimal-Humid (100%)";
...
void controlExhaustTemperature(float temperature)
{
    ...
    else
    {
        ledcWrite(EXHAUST_PIN, 255);
        exhaustServo.write(90);
        exhaustStatus = "Maksimal-Temp (100%)";
...
void applyFuzzyLogicControls()
{
    ...
    Serial.println(" Output - Exhaust: " + String(outputExhaust, 1));
    controlHeater(outputPemanas);
    controlFan(outputKipas);
    controlExhaust(outputExhaust);
...
void controlFan(float output)
{
    ...
    ledcWrite(FANHEATPAN1, duty); // Fan Panel
    bool kondisiCerah = (weatherMain == "cerah" || weatherMain == "berawan");
    time_t now = time(nullptr);
    struct tm *ptm = localtime(&now);
    int jamSekarang = ptm->tm_hour;
...
void controlExhaust(float output)
{
    int duty = constrain((int)round(output * 2.55), 0, 255);
    int persen = map(duty, 0, 255, 0, 100);

    ledcWrite(EXHAUST_PIN, duty);

    if (output <= 30)
    {
        exhaustServo.write(0);
        exhaustStatus = "Tutup (" + String(persen) + "%)";
    }
    else if (output > 30 && output <= 70)
    {
        exhaustServo.write(45);
        exhaustStatus = "Sedang (" + String(persen) + "%)";
    }
    else
    {
        exhaustServo.write(90);
        exhaustStatus = "Buka (" + String(persen) + "%)";
    }

    Serial.println("Exhaust Fan PWM: " + String(persen) + "% (" + String(duty) + "/255)");
    Serial.println("Exhaust Servo: " + exhaustStatus);
}
...
void controlSteamPusher(bool state)
{
    ...
    {
        ledcWrite(PUSHER_PIN, 255);
        Serial.println("Steam Pusher: ON");
    }
    else
    {
        ledcWrite(PUSHER_PIN, 0);
...
void setupFirebase()
{
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        else
        {
            ...
            Serial.println("X Firebase connection ERROR!");
        }
    }
}
...
void sendToFirebase()
{
    ...
    {
        Serial.println("X Suhu failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setFloat(firebaseData, "/sensor/humidity", kelembapanSensor))
    {
        Serial.println("X Humidity failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setFloat(firebaseData, "/sensor/kadarAir", kadarAir))
    {
        Serial.println("X KadarAir failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setFloat(firebaseData, "/sensor/berat", beratSekarang))
    {
        Serial.println("X Berat failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    // Send status data
    ...
    {
        Serial.println("X Status pengeringan failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setString(firebaseData, "/status/pemanas", pemanasStatus))
    {
        Serial.println("X Status pemanas failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setString(firebaseData, "/status/kipas", kipasStatus))
    {
        Serial.println("X Status kipas failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setString(firebaseData, "/status/exhaust", exhaustStatus))
    {
        Serial.println("X Status exhaust failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setBool(firebaseData, "/status/door_open", doorOpen))
    {
        Serial.println("X Door open status failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setBool(firebaseData, "/status/temp_protection", tempProtectionActive))
    {
        Serial.println("X Temp protection status failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setBool(firebaseData, "/status/humidity_control", humidityControlActive))
    {
        Serial.println("X Humidity control status failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    if (!Firebase.setBool(firebaseData, "/status/temperature_control", temperatureControlActive))
    {
        Serial.println("X Temperature control status failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    // Send control mode
    ...
    {
        Serial.println("X Auto mode status failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
    ...
    if (systemRunning && !sessionId.isEmpty())
    {
        ...
        {
            Serial.println("X Session data failed: " + firebaseData.errorReason());
            allSuccess = false;
        }
        delay(50);
    }
    ...
    {
        Serial.println("X System last update failed: " + firebaseData.errorReason());
        allSuccess = false;
    }
    delay(50);
...
void syncOfflineData()
{
    ...
    for (auto &dp : offlineBuffer)
    {
        ...
        {
            Serial.println("X Failed to sync: " + dp.timestamp);
            break;
        }
        delay(100);
    }
...
void setupFuzzyRules()
{
    // R1: IF Dingin AND Kering THEN Mati AND Mati AND Tutup
    ...
    r1_consequent->addOutput(tutupExhaust);
    FuzzyRule *rule1 = new FuzzyRule(1, r1_antecedent, r1_consequent);
    fuzzy->addFuzzyRule(rule1);

    // R2: IF Dingin AND Normal THEN Panas AND Cepat AND Sedang
    ...
    r2_consequent->addOutput(sedangExhaust);
    FuzzyRule *rule2 = new FuzzyRule(2, r2_antecedent, r2_consequent);
    fuzzy->addFuzzyRule(rule2);

    // R3: IF Dingin AND Lembab THEN Panas AND Cepat AND Sedang
    ...
    r3_consequent->addOutput(sedangExhaust);
    FuzzyRule *rule3 = new FuzzyRule(3, r3_antecedent, r3_consequent);
    fuzzy->addFuzzyRule(rule3);

    // R4: IF Normal AND Kering THEN Mati AND Mati AND Tutup
    ...
    r4_consequent->addOutput(tutupExhaust);
    FuzzyRule *rule4 = new FuzzyRule(4, r4_antecedent, r4_consequent);
    fuzzy->addFuzzyRule(rule4);

    // R5: IF Normal AND Normal THEN Hangat AND Lambat AND Sedang
    ...
    r5_consequent->addOutput(sedangExhaust);
    FuzzyRule *rule5 = new FuzzyRule(5, r5_antecedent, r5_consequent);
    fuzzy->addFuzzyRule(rule5);

    // R6: IF Normal AND Lembab THEN Hangat AND Lambat AND Sedang
    ...
    r6_consequent->addOutput(sedangExhaust);
    FuzzyRule *rule6 = new FuzzyRule(6, r6_antecedent, r6_consequent);
    fuzzy->addFuzzyRule(rule6);

    // R7: IF Panas AND Kering THEN Mati AND Mati AND Buka
    ...
    r7_consequent->addOutput(bukaExhaust);
    FuzzyRule *rule7 = new FuzzyRule(7, r7_antecedent, r7_consequent);
    fuzzy->addFuzzyRule(rule7);

    // R8: IF Panas AND Normal THEN Mati AND Lambat AND Sedang
    ...
    r8_consequent->addOutput(sedangExhaust);
    FuzzyRule *rule8 = new FuzzyRule(8, r8_antecedent, r8_consequent);
    fuzzy->addFuzzyRule(rule8);

    // R9: IF Panas AND Lembab THEN Mati AND Cepat AND Buka
    ...
    r9_consequent->addOutput(bukaExhaust);
...
void addError(String code, String message)
{
    ...
    if (errorCount < 10)
    {
        ...
        Serial.println("X ERROR ADDED: [" + code + "] " + message);
        if (WiFi.status() == WL_CONNECTED)
        {
...
void updateDefaultDisplay()
{
    ...
    case RUNNING:
        ...
        while (statusLine.length() < 16)
            statusLine += " ";
        if (statusLine.length() > 16)
            statusLine = statusLine.substring(0, 16);
...
void cycleInfoDisplay()
{
    ...
    else
    {
        ...
        {
        case INFO_SENSORS:
            currentInfoMode = INFO_WEIGHTS;
            break;
        case INFO_WEIGHTS:
            currentInfoMode = INFO_WIFI;
            break;
        case INFO_WIFI:
            currentInfoMode = INFO_HUMADITY;
            break;
        case INFO_HUMADITY:
            currentInfoMode = INFO_TEMP;
            break;
        case INFO_TEMP:
            currentInfoMode = INFO_HISTORY;
            break;
        case INFO_HISTORY:
            currentInfoMode = INFO_SENSORS;
            break;
    ...
    Serial.println("Info display mode: " + String(currentInfoMode));
}

//=============================================================================== Testing ===============================================================================
void testAllOutputs()
{
    Serial.println(" Testing HEATER 1 (Heater Warm)...");
    digitalWrite(HEATER1_PIN, HEATER_ON);
    delay(1000);
    digitalWrite(HEATER1_PIN, HEATER_OFF);
    Serial.println(" Testing HEATER 2 (Heater Hot)...");
    digitalWrite(HEATER2_PIN, HEATER_ON);
    delay(1000);
    digitalWrite(HEATER2_PIN, HEATER_OFF);
...
void testDHTSensor()
{
    ...
    if (isnan(testTemp))
    {
        ...
        Serial.println("X DHT22 temperature sensor ERROR");
    }
    else
    {
        clearError("DHT22_TEMP_FAILED");
    ...
    if (isnan(testHumid))
    {
        ...
        Serial.println("X DHT22 humidity sensor ERROR");
    }
    else
    {
        clearError("DHT22_HUMID_FAILED");
...
void testLoadCell()
{
    ...
    else
    {
        ...
        Serial.println("X HX711 load cell ERROR");
    }
}